---
name: implement
description: "Implements features based on a completed plan.md. Handles feature branch creation, task-by-task coding, testing, and PR creation. Supports pause/resume via progress.md state tracking. Use when starting implementation."
...
- strategy.status が "pending" なら警告:「/strategy を先に実行すると PR分割を計画できます」（実行は続行）。
- feature-state.json が存在しない場合は既存ロジックのまま続行。
...
2. ライブラリごとに library-researcher で調査（複数あれば並列実行）:
   ...
   Task(subagent_type: library-researcher):
     ...
     結果はMarkdown形式で返してください。」
   ```

3. 調査結果を **ライブラリごとに個別ファイルへ保存**:
   - 保存先: `docs/plans/{feature-name}/libs/{ライブラリ名}.md`
   - ファイル名はライブラリのパッケージ名をそのまま使う（例: `next-auth.md`, `prisma.md`）
   - スコープ付きパッケージは `/` を `-` に置換（例: `@shopify/polaris` → `shopify-polaris.md`）
...
- **progress.md が存在しない** → strategy-writer で自動生成:
  ...
  ※ strategy 未実施フラグにより「現在の状況」は「strategy 完了。」を省略、「作業ログ」は「1 PR で実装開始」とする
