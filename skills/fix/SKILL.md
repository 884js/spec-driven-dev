...
コードトレース（2-a）だけで原因が特定できない場合、以下を並列で起動する:

```
Task(subagent_type: analyzer):
  プロンプト: 「以下の不具合に関連する環境・設定情報を調査してください。
  症状: {症状の概要}
  調査観点:
  - 環境変数・設定ファイルの値
  - パッケージバージョンと依存関係
...
Task(subagent_type: researcher):
  プロンプト: 「以下の技術について、この不具合に関連する情報を調査してください。
  対象: {症状に関連するライブラリ・フレームワーク・プラットフォーム}
  調査観点:
  - 既知のバグ・Issue
  - バージョン固有の制約・破壊的変更
  - 正しい使い方・公式ドキュメントでの注意事項
  - 類似の不具合報告と解決策」
```

両方の結果を突合し、コード側の問題か環境・外部要因かを切り分ける。
