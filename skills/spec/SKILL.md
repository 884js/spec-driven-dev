...
Step 5: feature-state.json 更新 + 次のアクション提示
```
...
### feature-state.json の更新

新規モードの場合、feature-state.json を生成する。phase を "spec"、spec.status を "in_progress" に設定し、完了時に spec.status を "done"、phase を "build" に設定する。
